...
export const Header = () => {
  return (
    <header className="bg-gradient-to-tl from-[#000000] to-[#ffffff] w-screen">
        ...
        <ul className="items-center gap-2 hidden lg:flex lg:space-x-4">
          {links.map((link) => ( 
            <Link
              to={link.url}
              key={link.id}
              className="hover:border-b text-white font-normal tracking-wide hover:border-white ease-linear duration-150 transform transition-all hover:scale-110"
            >
              {link.title}
            </Link>
          ))}
